      ...
      ${result.review_items.map((item, index) => `
        <article class="quiz-card rounded-4 p-4 d-grid gap-3">
          <div class="d-flex justify-content-between gap-3 flex-wrap">
            <div>
              <div class="small text-uppercase text-secondary mb-2">Вопрос ${index + 1}</div>
              <h2 class="h4 mb-0">${quizApp.escapeHtml(item.question_text)}</h2>
            </div>
            <span class="quiz-chip ${item.is_correct ? "difficulty-beginner" : "difficulty-advanced"}">${item.is_correct ? "Верно" : "Ошибка"}</span>
          </div>
          <div class="row g-3">
            <div class="col-12 col-xl-6">
              <div class="small text-uppercase text-secondary mb-2">Ваш ответ</div>
              <div class="quiz-card rounded-4 p-3">${quizApp.escapeHtml(item.selected_option_text || "Ответ не отправлен")}</div>
            </div>
            <div class="col-12 col-xl-6">
              <div class="small text-uppercase text-secondary mb-2">Правильный ответ</div>
              <div class="quiz-card rounded-4 p-3 border-success-subtle">${quizApp.escapeHtml(item.correct_option_text)}</div>
            </div>
          </div>
          <div>
            <div class="small text-uppercase text-secondary mb-2">Пояснение</div>
            <div class="text-secondary">${quizApp.escapeHtml(item.explanation || "Пояснение не указано.")}</div>
          </div>
        </article>
      `).join("")}
    `;
